/* CLI / terminal-style widgets for smaller projects.
   Each widget tells the project's story through animated command-line output:
   typed prompts, streaming results, ASCII tables, progress bars, sparklines.
   All keyed on window.WIDGETS.<slug>. */

const { useState: cs, useEffect: ce, useRef: cR, useMemo: cM } = React;

/* ---------- Shared chrome + helpers ---------- */
function Term({ title, children, height = 360 }) {
  return (
    <div className="rounded-2xl overflow-hidden font-mono text-[12px]" style={{ border: '1px solid var(--border-strong)', background: 'var(--bg)' }}>
      ...
      <div className="p-5 overflow-hidden" style={{ minHeight: height }}>
        {children}
      </div>
    </div>
  );
...
function useTypewriter(items, speed = 80) {
  const [step, setStep] = cs(0);
  ce(() => {
    if (step >= items.length) return;
    const t = setTimeout(() => setStep((s) => s + 1), speed + (items[step]?.pause || 0));
    return () => clearTimeout(t);
  }, [step, items, speed]);
  return [step, () => setStep(0)];
}

function Caret({ on = true }) {
...
const C = {
  ...
  ok: { color: 'oklch(0.75 0.18 145)' },
  warn: { color: 'oklch(0.8 0.15 80)' },
  err: { color: 'oklch(0.7 0.2 25)' },
};

/* =========================================================
   ATTIRE AI — fashion outfit recommender
   conversational CLI: ingest preferences → call LLM → ASCII outfit grid
   ========================================================= */
function AttireAIWidget() {
  const lines = [
  ...
  return (
    <Term title="attire-ai · style chat">
      {lines.slice(0, step).map((l, i) => (
        <div key={i} style={l.s}>{l.t}{l.inline && <span style={{ color: 'var(--text-primary)' }}>  {l.inline}</span>}</div>
      ))}
      {step < lines.length && <Caret />}
      {showOutfit && (
        <div className="mt-4 grid grid-cols-4 gap-3">
          {[
            { label: 'jacket', tone: 'oklch(0.5 0.04 60)', sub: 'oversized linen' },
            { label: 'tee', tone: 'oklch(0.92 0.02 80)', sub: 'cream pima' },
            { label: 'trouser', tone: 'oklch(0.45 0.04 250)', sub: 'pleated wool' },
            { label: 'shoe', tone: 'oklch(0.85 0.06 75)', sub: 'leather sneaker' },
          ].map((it) => (
            <div key={it.label} className="rounded-md overflow-hidden" style={{ border: '1px solid var(--border)' }}>
              ...
              <div className="p-2 text-[10px]">
                ...
                <div style={{ color: 'var(--text-muted)' }}>{it.sub}</div>
              </div>
            </div>
          ))}
        </div>
...
/* =========================================================
   COUPON VAULT — promotion engine REPL
   ========================================================= */
function CouponVaultWidget() {
  const lines = [
...
/* =========================================================
   REFLECTIONS — AI-augmented blog editor draft
   ========================================================= */
function ReflectionsWidget() {
  ...
  const [n, setN] = cs(0);
  ce(() => { if (n >= draftWords.length) return; const t = setTimeout(() => setN(n + 1), 90); return () => clearTimeout(t); }, [n, draftWords.length]);
  return (
...
/* =========================================================
   POWER UP — Wear OS health log
   ========================================================= */
function PowerUpWidget() {
  const [t, setT] = cs(0);
  ce(() => { const i = setInterval(() => setT(v => v + 1), 220); return () => clearInterval(i); }, []);
  ...
  return (
    <Term title="powerup · wear-os sync" height={320}>
      ...
      <div className="mt-4 grid grid-cols-3 gap-3">
        {[
          { k: 'heart', v: hr, u: 'bpm', sym: '♥' },
          { k: 'steps', v: steps.toLocaleString(), u: '/ 10,000', sym: '⌁' },
          { k: 'kcal',  v: cal,  u: 'kcal',  sym: '⚡' },
        ].map((m) => (
          <div key={m.k} className="rounded p-3" style={{ background: 'var(--bg-elev)', border: '1px solid var(--border)' }}>
            ...
            <div className="text-2xl mt-1" style={{ color: 'var(--accent)', fontFamily: 'JetBrains Mono' }}>{m.v}</div>
      ...
      <svg viewBox="0 0 200 30" className="w-full mt-4" style={{ height: 50 }}>
        <polyline
          points={Array.from({ length: 60 }).map((_, i) => `${i * 3.4},${15 - Math.sin((t + i) * 0.3) * 9 - Math.sin((t + i) * 0.13) * 4}`).join(' ')}
...
/* =========================================================
   EDUSYS — Outcome-Based Education mapper
   ========================================================= */
function EduSysWidget() {
  const lines = [
          ...
          <div className="grid grid-cols-7 gap-1 text-[11px]">
            ...
            {matrix.map((row) => (
              <React.Fragment key={row[0]}>
                ...
                {row.slice(1).map((v, j) => (
                  <div key={j} className="text-center rounded" style={{
                    background: v === 0 ? 'transparent' : `oklch(0.${30 + v * 10} 0.${5 + v * 4} 152 / ${0.2 + v * 0.25})`,
                    color: v === 0 ? 'var(--text-faint)' : 'var(--text-primary)',
                    padding: '6px 0',
                  }}>{v || '·'}</div>
                ))}
              </React.Fragment>
...
/* =========================================================
   FIRST PAPER — research recommender
   ========================================================= */
function FirstPaperWidget() {
  const lines = [
...
/* =========================================================
   THE ONE FINDER — match scoring with E2E chat preview
   ========================================================= */
function OneFinderWidget() {
  const [t, setT] = cs(0);
  ce(() => { const i = setInterval(() => setT(v => v + 1), 700); return () => clearInterval(i); }, []);
  const candidates = [
...
/* =========================================================
   HOME GINIE — house price predictor
   ========================================================= */
function HomeGinieWidget() {
  const [income, setIncome] = cs(78000);
  const [age, setAge] = cs(7);
  const [rooms, setRooms] = cs(6);
  // toy linear model
  ...
  return (
    <Term title="home-ginie · regression" height={340}>
      ...
      <div className="mt-4 grid grid-cols-2 gap-x-6 gap-y-3 text-[12px]">
        {[
          { l: 'avg income', v: income, set: setIncome, min: 30000, max: 200000, step: 1000, fmt: (v) => `$${v.toLocaleString()}` },
          { l: 'house age',  v: age,    set: setAge,    min: 0,     max: 60,     step: 1,    fmt: (v) => `${v} y` },
          { l: 'rooms',      v: rooms,  set: setRooms,  min: 1,     max: 12,     step: 1,    fmt: (v) => `${v}` },
        ].map((f) => (
          <div key={f.l}>
            <div className="flex justify-between">
              <span style={C.out}>{f.l}</span>
              <span style={{ color: 'var(--accent)' }}>{f.fmt(f.v)}</span>
            </div>
...
/* =========================================================
   MOVIESCAPE — streaming TMDB results
   ========================================================= */
function MoviescapeWidget() {
  const movies = [
  ...
  const [n, setN] = cs(0);
  ce(() => { if (n >= movies.length) return; const t = setTimeout(() => setN(n + 1), 320); return () => clearTimeout(t); }, [n, movies.length]);
  return (
...
/* =========================================================
   EMOJI NATION — emoji search
   ========================================================= */
function EmojiNationWidget() {
  const [q, setQ] = cs('joy');
  const all = {
    joy:    ['😂','🤣','😄','😁','😆','😊','🥳','🎉','🥰','✨'],
    fire:   ['🔥','🔆','🕯️','🎆','🎇','💥','♨️','🌶️','🚒','🧨'],
  ...
  return (
    <Term title="emoji-nation · search" height={300}>
      ...
      <div className="grid grid-cols-10 gap-2 mt-4">
        {list.map((e, i) => (
          <div key={i} className="aspect-square flex items-center justify-center rounded text-2xl smooth" style={{ background: 'var(--bg-elev)', border: '1px solid var(--border)' }}>
            {e}
          </div>
        ))}
      </div>
...
/* =========================================================
   HEALTHY ME — calorie + water log
   ========================================================= */
function HealthyMeWidget() {
  const lines = [
    ...
    <Term title="healthy-me · daily log" height={320}>
      ...
      {step >= lines.length && (
        <div className="mt-3 grid grid-cols-2 gap-3">
          {[
            { k: 'calories', v: 1480, max: 2200, u: 'kcal' },
            { k: 'water',    v: 1400, max: 2500, u: 'ml'   },
            { k: 'protein',  v: 64,   max: 110,  u: 'g'    },
            { k: 'sugar',    v: 38,   max: 50,   u: 'g'    },
          ].map((m) => {
            const pct = Math.min(1, m.v / m.max);
            return (
              <div key={m.k} className="rounded p-3" style={{ background: 'var(--bg-elev)', border: '1px solid var(--border)' }}>
                <div className="flex justify-between text-[11px]">
                  <span style={C.out}>{m.k}</span>
                  <span style={{ color: 'var(--accent)' }}>{m.v} / {m.max} {m.u}</span>
                </div>
                <div className="h-1.5 mt-2 rounded overflow-hidden" style={{ background: 'var(--border)' }}>
...
/* =========================================================
   PROCTOR IT — face-detect / eye-tracking simulator
   ========================================================= */
function ProctorItWidget() {
  const [t, setT] = cs(0);
  ce(() => { const i = setInterval(() => setT(v => v + 1), 90); return () => clearInterval(i); }, []);
      ...
      <div className="grid grid-cols-2 gap-4 mt-3">
        ...
        <div className="space-y-2 text-[12px]">
          <div className="flex justify-between border-b py-1" style={{ borderColor: 'var(--border)' }}>
            <span style={C.out}>gaze drift</span>
            <span style={flag ? C.warn : C.ok}>{eye.toFixed(2)}°</span>
          </div>
          <div className="flex justify-between border-b py-1" style={{ borderColor: 'var(--border)' }}>
            <span style={C.out}>blink rate</span>
            <span style={C.ok}>14 / min</span>
          </div>
          <div className="flex justify-between border-b py-1" style={{ borderColor: 'var(--border)' }}>
            <span style={C.out}>head pose</span>
            <span style={C.ok}>0.94 forward</span>
          </div>
          <div className="flex justify-between border-b py-1" style={{ borderColor: 'var(--border)' }}>
            <span style={C.out}>focus score</span>
            <span style={{ color: 'var(--accent)' }}>{flag ? '0.62 ⚠' : '0.91 ✓'}</span>
          </div>
...
/* =========================================================
   WEATHER TODAY — typed forecast
   ========================================================= */
function WeatherTodayWidget() {
  const lines = [
      ...
      {step >= lines.length && (
        <>
          <div className="mt-3 flex items-baseline gap-4">
            ...
            <div>
              <div style={C.user}>partly cloudy</div>
              <div style={C.out} className="text-[11px]">feels 12° · humidity 64% · wind 14 km/h NE</div>
            </div>
          </div>
...
/* =========================================================
   RANDOM QUOTE GENERATOR — color-shifting card
   ========================================================= */
function RandomQuoteWidget() {
  ...
  const [i, setI] = cs(0);
  const [hue, setHue] = cs(152);
  function next() {
    setI((i + 1) % quotes.length);
    setHue(Math.floor(Math.random() * 360));
  }
  ce(() => { const id = setInterval(next, 3500); return () => clearInterval(id); /* eslint-disable-next-line */ }, [i]);
  const cur = quotes[i];
  const bg = `oklch(0.${22} 0.06 ${hue})`;
  const fg = `oklch(0.85 0.12 ${hue})`;
  return (
    <Term title="random-quote · color shift" height={340}>
      ...
      <div className="mt-4 rounded-xl p-6 smooth" style={{ background: bg, border: `1px solid ${fg}`, transition: 'all 0.6s var(--ease)' }} key={i}>
        <div className="font-serif text-[22px] leading-snug" style={{ color: fg }}>"{cur.q}"</div>
        <div className="mt-3 font-mono text-[11px]" style={{ color: fg, opacity: 0.7 }}>— {cur.a}</div>
      </div>
...
/* ---------- export ---------- */
Object.assign(window.WIDGETS || (window.WIDGETS = {}), {
  attireAi:      AttireAIWidget,
  couponVault:   CouponVaultWidget,
  reflections:   ReflectionsWidget,
  powerUp:       PowerUpWidget,
  edusys:        EduSysWidget,
  firstPaper:    FirstPaperWidget,
  oneFinder:     OneFinderWidget,
  homeGinie:     HomeGinieWidget,
  moviescape:    MoviescapeWidget,
  emojiNation:   EmojiNationWidget,
  healthyMe:     HealthyMeWidget,
  proctorIt:     ProctorItWidget,
  weatherToday:  WeatherTodayWidget,
  randomQuote:   RandomQuoteWidget,
});
