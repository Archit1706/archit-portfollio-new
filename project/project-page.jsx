/* global React, ReactDOM, PROJECT_BY_SLUG, PROJECTS, WIDGETS */
const { useState: ps, useEffect: pe, useMemo: pm } = React;

function ProjectPage() {
  const [theme, setTheme] = ps(() => localStorage.getItem('ar-theme') || 'dark');
  ...
  const slug = pm(() => new URLSearchParams(location.search).get('p'), []);
  const p = window.PROJECT_BY_SLUG[slug];
  ...
  if (!p) {
    return (
      <div className="min-h-screen flex items-center justify-center font-mono text-[12px]" style={{ color: 'var(--text-muted)' }}>
        project not found · <a href="projects.html" className="ml-2" style={{ color: 'var(--accent)' }}>back to index</a>
      </div>
    );
  }

  const Widget = window.WIDGETS[p.widget] || window.WIDGETS.terminal;

  return (
    <>
      <CustomCursor />
      <div className="fixed inset-0 grid-paper pointer-events-none" style={{ opacity: 0.25, zIndex: 0 }} />
      ...
      <div className="relative" style={{ zIndex: 1 }}>
        ...
        <nav className="px-6 md:px-10 py-6 flex items-center justify-between sticky top-0 backdrop-blur-md" style={{ background: 'color-mix(in oklab, var(--bg) 75%, transparent)', borderBottom: '1px solid var(--border)', zIndex: 50 }}>
          <a href="projects.html" className="flex items-center gap-3 font-mono text-[12px] uppercase tracking-[0.18em]" data-hover>
          ...
          <div className="flex items-center gap-3 font-mono text-[11px]">
            <a href="index.html" className="px-3 py-1.5" style={{ color: 'var(--text-muted)' }} data-hover>home</a>
        ...
        <header className="px-6 md:px-10 max-w-[1400px] mx-auto pt-16 pb-12">
          <div className="font-mono text-[11px] uppercase tracking-[0.18em] mb-6" style={{ color: 'var(--accent)' }}>
            <span style={{ color: 'var(--text-muted)' }}>{String(idx + 1).padStart(2,'0')} / {window.PROJECTS.length}</span> &nbsp;·&nbsp; {p.category}
            {p.featured && <>&nbsp;·&nbsp;<span>featured</span></>}
          </div>
          <h1 className="font-serif" style={{ fontSize: 'clamp(40px, 7vw, 96px)', lineHeight: 0.95, letterSpacing: '-0.03em', textWrap: 'balance' }}>
            {p.title}
          </h1>
          <p className="mt-6 font-serif text-[24px] md:text-[28px] leading-snug max-w-[42ch]" style={{ color: 'var(--text-muted)', textWrap: 'pretty' }}>
            {p.subtitle}
          </p>
          <div className="mt-8 flex flex-wrap gap-2">
            {p.link && (
              <a href={p.link} target="_blank" rel="noreferrer" className="smooth px-4 py-2.5 rounded-md font-mono text-[11px] flex items-center gap-2 uppercase tracking-[0.12em]" style={{ background: 'var(--accent)', color: 'var(--bg)' }} data-hover>
                live ↗
              </a>
            )}
            {p.github && (
              <a href={p.github} target="_blank" rel="noreferrer" className="smooth px-4 py-2.5 rounded-md font-mono text-[11px] flex items-center gap-2 uppercase tracking-[0.12em]" style={{ border: '1px solid var(--border-strong)' }} data-hover>
                source ↗
              </a>
            )}
          </div>
        ...
        <section className="px-6 md:px-10 max-w-[1400px] mx-auto pb-20 grid md:grid-cols-12 gap-10">
          ...
          <div className="md:col-span-7">
            ...
            <p className="font-serif text-[20px] leading-relaxed" style={{ textWrap: 'pretty' }}>
              {p.desc}
            </p>

            {p.features && (
              <div className="mt-12">
                <div className="font-mono text-[10px] uppercase tracking-[0.18em] mb-4" style={{ color: 'var(--text-muted)' }}>what it does</div>
                <ul className="space-y-2">
                  {p.features.map((f, i) => (
                    <li key={i} className="flex gap-3 text-[16px]" style={{ color: 'var(--text-primary)' }}>
                      <span className="font-mono text-[11px] mt-1" style={{ color: 'var(--accent)' }}>{String(i + 1).padStart(2, '0')}</span>
          ...
          <aside className="md:col-span-5 md:pl-6 md:border-l space-y-6" style={{ borderColor: 'var(--border)' }}>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-1.5">
                {p.tech?.map((t) => (
                  <span key={t} className="font-mono text-[11px] px-2 py-1 rounded" style={{ background: 'var(--bg-elev)', border: '1px solid var(--border)', color: 'var(--text-primary)' }}>
                    {t}
                  </span>
                ))}
              </div>
        ...
        <section className="px-6 md:px-10 max-w-[1400px] mx-auto pb-32">
          <div className="grid md:grid-cols-2 gap-4 border-t pt-8" style={{ borderColor: 'var(--border)' }}>
            <a href={`project.html?p=${prev.slug}`} className="group block p-6 rounded-xl smooth" style={{ border: '1px solid var(--border)' }} data-hover>
              <div className="font-mono text-[10px] uppercase tracking-[0.18em]" style={{ color: 'var(--text-muted)' }}>← previous</div>
              <div className="font-serif text-2xl mt-2 group-hover:text-[var(--accent)] smooth">{prev.title}</div>
              <div className="font-mono text-[11px] mt-1" style={{ color: 'var(--text-muted)' }}>{prev.subtitle}</div>
            </a>
            <a href={`project.html?p=${next.slug}`} className="group block p-6 rounded-xl smooth text-right" style={{ border: '1px solid var(--border)' }} data-hover>
              <div className="font-mono text-[10px] uppercase tracking-[0.18em]" style={{ color: 'var(--text-muted)' }}>next →</div>
              <div className="font-serif text-2xl mt-2 group-hover:text-[var(--accent)] smooth">{next.title}</div>
        ...
        <footer className="px-6 md:px-10 py-12 border-t" style={{ borderColor: 'var(--border)' }}>
          <div className="max-w-[1400px] mx-auto flex flex-wrap gap-4 justify-between font-mono text-[11px]" style={{ color: 'var(--text-muted)' }}>
            ...
            <a href="projects.html" data-hover>view all projects ↗</a>
          </div>
        </footer>
      </div>
    </>
