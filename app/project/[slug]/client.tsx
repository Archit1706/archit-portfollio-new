...
type WC = React.ComponentType<{ project: Project }>;
type WidgetModule = Record<string, React.ComponentType<{ project: Project }>>;

const WidgetLoading = () => (
  <div
    className="w-full rounded-xl flex items-center justify-center font-mono text-[12px]"
    style={{ height: 360, border: '1px solid var(--border)', color: 'var(--text-muted)' }}
  >
    loading demo…
  </div>
);

function pw(name: string): WC {
  return dynamic<{ project: Project }>(
    () =>
      import('@/components/widgets/project-widgets').then((m) => ({
...
function cw(name: string): WC {
  return dynamic<{ project: Project }>(
    () =>
      import('@/components/widgets/cli-widgets').then((m) => ({
        default: (m as unknown as WidgetModule)[name],
      })),
    { loading: WidgetLoading }
  ) as WC;
...
const WIDGET_MAP: Record<string, WC> = {
  ...
  panorama:    pw('PanoramaWidget'),
  tonePitch:   pw('TonePitchWidget'),
  urlScanner:  pw('URLScannerWidget'),
  pose:        pw('PoseWidget'),
  audioSent:   pw('AudioSentWidget'),
  network:     pw('NetworkWidget'),
  auction:     pw('AuctionWidget'),
  terminal:    pw('TerminalFallbackWidget'),
  attireAi:    cw('AttireAIWidget'),
  couponVault: cw('CouponVaultWidget'),
  reflections: cw('ReflectionsWidget'),
  powerUp:     cw('PowerUpWidget'),
  edusys:      cw('EduSysWidget'),
  firstPaper:  cw('FirstPaperWidget'),
  oneFinder:   cw('OneFinderWidget'),
  homeGinie:   cw('HomeGinieWidget'),
  moviescape:  cw('MoviescapeWidget'),
  emojiNation: cw('EmojiNationWidget'),
  healthyMe:   cw('HealthyMeWidget'),
  proctorIt:   cw('ProctorItWidget'),
  weatherToday: cw('WeatherTodayWidget'),
  randomQuote: cw('RandomQuoteWidget'),
};
